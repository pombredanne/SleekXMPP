...
class XMLStream(object):
    ...
    def __init__(self, socket=None, host='', port=0, escape_quotes=False, connection_type='client'):
    ...
    def connect(self, host='', port=0, use_ssl=False, use_tls=True):
        "clientConnect"
        return self.clientConnect(host, port, use_ssl, use_tls)

    def clientConnect(self, host='', port=0, use_ssl=None, use_tls=None, reattempt=True):
        "Connect and create socket"
        if host and port:
            self.address = (host, int(port))
        if use_ssl is not None:
            self.use_ssl = use_ssl
        self.state.setState('connection', 'disconnected', 'connected')

    def __connect(self):
        """Triggered only by state change"""
        if not self.connection_type == 'client':
            return False
        if sys.version_info < (3, 0):
